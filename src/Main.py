...
monster0.load(monster0_png, 100, 100, 4,300,400)
...
def game():
    mapnum=0
    maplist = load_map('map' + str(mapnum) + '.txt')
    while True:
        framerate.tick(30)
        ticks = pygame.time.get_ticks()                 # pygame初始化以来至现在的毫秒数
        # 将背景图画上去
        map = Map(hero0.herox, hero0.heroy,mapnum,maplist)
        judge = Judge(hero0.herox,hero0.heroy,map.maplist)
    ...
    # 控制人物移动
        ...
        if keys[K_w]:
            if(judge.up()):
                hero0.heroy = hero0.heroy-7
                monster0.velocity[1] = -7
            hero0.movement = True
        if keys[K_s]:
            if (judge.down()):
                hero0.heroy = hero0.heroy+7
        ...
        if keys[K_d]:
            if (judge.right()):
                hero0.herox = hero0.herox+7
                monster0.velocity[0] = 7
            hero0.movement = True
            hero0.direction = 1
        if keys[K_a]:
            if (judge.left()):
            ...
            hero0.movement = True
            hero0.direction = 0
        print(hero0.heroy,hero0.herox)
        if(choss(mapnum,map,hero0)):#选择关卡,返回关卡数
            mapnum = choss(mapnum,map,hero0)
            hero0.herox = 540
            hero0.heroy = 360
            maplist = load_map('map' + str(mapnum) + '.txt')
        # 控制开火
